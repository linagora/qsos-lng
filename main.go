...
var (
	...
	securityWeights = map[string]int64{
		...
		"Vulnerabilities": 3, // Only known vulnerabilities, so it may give better scores for less known projects
		// "Dependency-Update-Tool" is disabled, as it gives 0 for projects with no dependencies
		...
		// "Fuzzing" is disabled, as it does not detect enough tools
		// "SAST" is disabled, as it does not detect enough tools
		"Binary-Artifacts":  3,
		"Branch-Protection": 3,
		// "Dangerous-Workflow" is disabled, as it does not look mature enough
		"Code-Review": 3,
		// "Contributors" is disabled, as it's already in the community section
		"Pinned-Dependencies": 2,
		// "Token-Permissions" id disabled, as it does not look mature enough
		"Packaging":       2,
		"Signed-Releases": 3,
	}
)
